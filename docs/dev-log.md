...
- **AI Recipe Regeneration**: Added a "Regenerate" button in the generated recipe dialog (recently redesigned and moved to a sticky Dialog Footer for a cleaner UI, with enlarged `h-11` primary buttons and a soft blue `bg-sky-100` background for better aesthetic differentiation). This leverages a new AI Prompt engineering technique by passing `previouslyUsedIngredients` to the backend, instructing Gemini to actively down-weight previous ingredients and explore diverse recipe combinations while maintaining required ingredients.
